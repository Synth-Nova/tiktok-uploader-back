  ...
  try {
    ...
    if (active.length > 0) {
      ...
      for (const job of active) {
        ...
        log(`ID: ${job.id}`);
        log(`  Хэштег: #${job.data.hashtag}`);
        log(`  Аккаунт ID: ${job.data.accountId}`);
        log(`  Прогресс: ${progress}%`);
        log(`  Попытка: ${job.attemptsMade + 1}/${job.opts.attempts || 1}`);
        if (job.data.proxy) {
          log(`  Прокси: ${job.data.proxy}`);
        }
        log("-" .repeat(80));
      }
      log("");
    ...
    log("  - Worker обрабатывает по 1 задаче одновременно");
    log("  - Каждая задача повторяется 1 раз при ошибке");
    log("  - Прогресс сохраняется в базу данных в таблицу stats_progress");
    log("=" .repeat(80));
    ...
    log(`❌ Ошибка при просмотре очереди статистики: ${error.message}`);
    await statsQueue.close();
    process.exit(1);
  }
}
